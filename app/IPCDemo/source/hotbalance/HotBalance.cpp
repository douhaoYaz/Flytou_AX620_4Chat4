...
AX_BOOL CHotBalance::Escape(AX_VOID)
{
    ...
    if (m_tHotBalanceConfig.tCameraConf[AX_DEV_SOURCE_SNS_ID].bValid) {
        m_nShutterMode = g_camera.GetShutterMode();

        // shutter mode
        if (m_nShutterMode != 0) {
            // Set to FIX FRAME RATE MODE
            g_camera.SetShutterMode(0);
        }
...
AX_BOOL CHotBalance::Recovery(AX_VOID)
{
    ...
    // recovery to shutter mode
    if (m_nShutterMode != 0) {
        g_camera.SetShutterMode(m_nShutterMode);
    }
