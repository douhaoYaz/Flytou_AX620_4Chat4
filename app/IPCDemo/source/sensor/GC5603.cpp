...
#define GC5603 "GC5603"

extern COptionHelper gOptions;

CGC5603::CGC5603(SENSOR_CONFIG_T tSensorConfig)
    : CBaseSensor(tSensorConfig)
{
}

CGC5603::~CGC5603(AX_VOID)
{
}

AX_VOID CGC5603::InitSnsAttr()
{
    /* Referenced by AX_VIN_SetSnsAttr */
    m_tSnsAttr.nWidth               = 2944;
    m_tSnsAttr.nHeight              = 1664;
...
AX_VOID CGC5603::InitMipiRxAttr()
{
...
AX_VOID CGC5603::InitDevAttr()
{
    ...
    m_tDevAttr.tDevImgRgn.nWidth  = 2944;
    m_tDevAttr.tDevImgRgn.nHeight = 1664;
...
AX_VOID CGC5603::InitPipeAttr()
{
    /* Referenced by AX_VIN_SetPipeAttr */
    m_tPipeAttr.nWidth          = 2944;
    m_tPipeAttr.nHeight         = 1664;
...
AX_VOID CGC5603::InitChnAttr()
{
    /* Referenced by AX_VIN_SetChnAttr */
    m_tChnAttr.tChnAttr[0].nWidth = 2944;
    m_tChnAttr.tChnAttr[0].nHeight = 1664;
    m_tChnAttr.tChnAttr[0].eImgFormat = AX_YUV420_SEMIPLANAR;
    m_tChnAttr.tChnAttr[0].bEnable = AX_TRUE;
    m_tChnAttr.tChnAttr[0].nWidthStride = 2944;
    m_tChnAttr.tChnAttr[0].nDepth = YUV_GC5603_DEPTH;
    ...
    m_tChnAttr.tChnAttr[1].nDepth = YUV_GC5603_DEPTH;

    m_tChnAttr.tChnAttr[2].nWidth = 1280;
    m_tChnAttr.tChnAttr[2].nHeight = 720;
    m_tChnAttr.tChnAttr[2].eImgFormat = AX_YUV420_SEMIPLANAR;
    m_tChnAttr.tChnAttr[2].bEnable = AX_TRUE;
    m_tChnAttr.tChnAttr[2].nWidthStride = 1280;
    m_tChnAttr.tChnAttr[2].nDepth = YUV_GC5603_DEPTH;
}

AX_S32 CGC5603::RegisterSensor(AX_U8 nPipe)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);

    AX_S32 nRet = 0;

    m_pSnsLib = dlopen("libsns_gc5603.so", RTLD_LAZY);
    if (NULL == m_pSnsLib) {
        LOG_M_E(GC5603, "Load sensor lib failed!");
        return -1;
    }

    m_pSnsObj = (AX_SENSOR_REGISTER_FUNC_T *)dlsym(m_pSnsLib, "gSnsgc5603Obj");
    if (NULL == m_pSnsObj) {
        LOG_M_E(GC5603, "AX_VIN Get Sensor Object Failed!");
        return -1;
    }

    nRet = AX_VIN_RegisterSensor(nPipe, m_pSnsObj);
    if (nRet) {
        LOG_M_E(GC5603, "AX_ISP Register Sensor Failed, ret=0x%x.", nRet);
        return nRet;
    }
    ...
    if (NULL != m_pSnsObj->pfn_sensor_set_bus_info) {
        nRet = m_pSnsObj->pfn_sensor_set_bus_info(nPipe, tSnsBusInfo);
        if (0 != nRet) {
            LOG_M_E(GC5603, "Sensor set bus info Failed, ret=0x%x.", nRet);
            return nRet;
        }
        LOG_M(GC5603, "set sensor bus idx %d", tSnsBusInfo.I2cDev);
    } else {
        LOG_M(GC5603, "not support set sensor bus info!");
        return -1;
    }
...
AX_S32 CGC5603::UnRegisterSensor(AX_U8 nPipe)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
...
AX_S32 CGC5603::RegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;
    ...
    if (nRet) {
        LOG_M_E(GC5603, "AX_ISP_RegisterAwbLibCallback Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CGC5603::UnRegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;

    nRet = AX_ISP_UnRegisterAwbLibCallback(nPipe);
    if (nRet) {
        LOG_M_E(GC5603, "AX_ISP Unregister Sensor Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CGC5603::SetFps(AX_U8 nPipe, AX_F32 fFrameRate)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
    ...
    if (m_pSnsObj && m_pSnsObj->pfn_sensor_set_fps) {
        ...
        if (nRet) {
            LOG_M_E(GC5603, "[%d] Set Sensor Fps  to %.1f failed, ret=0x%x", nPipe, fFrameRate, nRet);
            return nRet;
        }
        else {
            LOG_M(GC5603, "[%d] Set Sensor Fps to %.1f", nPipe, fFrameRate);
        }
...
AX_BOOL CGC5603::HdrModeSupport(AX_U8 nPipe)
{
    return AX_FALSE;
}
