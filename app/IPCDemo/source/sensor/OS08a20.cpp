...
#define OS08A20 "OS08A20"

extern COptionHelper gOptions;

COS08a20::COS08a20(SENSOR_CONFIG_T tSensorConfig)
...
COS08a20::~COS08a20(AX_VOID)
{
}

AX_VOID COS08a20::InitSnsAttr()
{
...
AX_VOID COS08a20::InitMipiRxAttr()
{
...
AX_VOID COS08a20::InitChnAttr()
{
    ...
    m_tChnAttr.tChnAttr[0].nWidthStride = 3840;
    ...
    m_tChnAttr.tChnAttr[2].nWidth = 960;
    m_tChnAttr.tChnAttr[2].nHeight = 540;
    m_tChnAttr.tChnAttr[2].eImgFormat = AX_YUV420_SEMIPLANAR;
    m_tChnAttr.tChnAttr[2].bEnable = AX_TRUE;
    m_tChnAttr.tChnAttr[2].nWidthStride = 960;
    m_tChnAttr.tChnAttr[2].nDepth = 5;
}

AX_S32 COS08a20::RegisterSensor(AX_U8 nPipe)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);

    AX_S32 nRet = 0;

    m_pSnsLib = dlopen("libsns_os08a20.so", RTLD_LAZY);
    if (NULL == m_pSnsLib) {
        LOG_M_E(OS08A20, "Load sensor lib failed!");
        return -1;
    }

    m_pSnsObj = (AX_SENSOR_REGISTER_FUNC_T *)dlsym(m_pSnsLib, "gSnsos08a20Obj");
    if (NULL == m_pSnsObj) {
        LOG_M_E(OS08A20, "AX_VIN Get Sensor Object Failed!");
        return -1;
    }

    nRet = AX_VIN_RegisterSensor(nPipe, m_pSnsObj); /* pipe 0 */
    if (nRet) {
        LOG_M_E(OS08A20, "AX_ISP Register Sensor Failed, ret=0x%x.", nRet);
        return nRet;
    }
    ...
    if (NULL != m_pSnsObj->pfn_sensor_set_bus_info) {
        ...
        if (0 != nRet) {
            LOG_M_E(OS08A20, "Sensor set bus info Failed, ret=0x%x.", nRet);
            return nRet;
        }
        LOG_M(OS08A20, "set sensor bus idx %d", tSnsBusInfo.I2cDev);
    } else {
        LOG_M(OS08A20, "not support set sensor bus info!");
        return -1;
    }
...
AX_S32 COS08a20::UnRegisterSensor(AX_U8 nPipe)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
...
AX_S32 COS08a20::RegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;
    ...
    if (nRet) {
        LOG_M_E(OS08A20, "AX_ISP_RegisterAwbLibCallback Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 COS08a20::UnRegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;

    nRet = AX_ISP_UnRegisterAwbLibCallback(nPipe);
    if (nRet) {
        LOG_M_E(OS08A20, "AX_ISP Unregister Sensor Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 COS08a20::SetFps(AX_U8 nPipe, AX_F32 fFrameRate)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
    ...
    if (m_pSnsObj && m_pSnsObj->pfn_sensor_set_fps) {
        ...
        if (nRet) {
            LOG_M_E(OS08A20, "[%d] Set Sensor Fps  to %.1f failed, ret=0x%x", nPipe, fFrameRate, nRet);
            return nRet;
        }
        else {
            LOG_M(OS08A20, "[%d] Set Sensor Fps to %.1f", nPipe, fFrameRate);
        }
...
AX_BOOL COS08a20::HdrModeSupport(AX_U8 nPipe)
{
    return AX_TRUE;
}
