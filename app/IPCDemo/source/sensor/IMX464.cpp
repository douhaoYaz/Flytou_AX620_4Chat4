...
#define IMX464 "IMX464"

extern COptionHelper gOptions;

CIMX464::CIMX464(SENSOR_CONFIG_T tSensorConfig)
    : CBaseSensor(tSensorConfig)
{
    m_eImgFormatSDR = AX_FORMAT_BAYER_RAW_12BPP;
    m_eImgFormatHDR = AX_FORMAT_BAYER_RAW_12BPP;
}

CIMX464::~CIMX464(AX_VOID)
{
}

AX_VOID CIMX464::InitSnsAttr()
{
    ...
    m_tSnsAttr.eSnsHcgLcg           = AX_LCG_MODE;
...
AX_VOID CIMX464::InitMipiRxAttr()
{
...
AX_VOID CIMX464::InitDevAttr()
{
    ...
    if (AX_SNS_HDR_2X_MODE == m_tDevAttr.eSnsMode) {
        m_tDevAttr.ePixelFmt      = m_eImgFormatHDR;
    } else {
        m_tDevAttr.ePixelFmt      = m_eImgFormatSDR;
    }
}

AX_VOID CIMX464::InitPipeAttr()
{
...
AX_VOID CIMX464::InitChnAttr()
{
    ...
    m_tChnAttr.tChnAttr[0].nDepth = YUV_IMX464_DEPTH;
   ...
   m_tChnAttr.tChnAttr[1].nDepth = YUV_IMX464_DEPTH;
    ...
    m_tChnAttr.tChnAttr[2].nDepth = YUV_IMX464_DEPTH;
}

AX_S32 CIMX464::RegisterSensor(AX_U8 nPipe)
{
    AX_S32 nRet = 0;

    m_pSnsLib = dlopen("libsns_imx464.so", RTLD_LAZY);
    if (NULL == m_pSnsLib) {
        LOG_M_E(IMX464, "Load sensor lib failed!");
        return -1;
    }

    m_pSnsObj = (AX_SENSOR_REGISTER_FUNC_T *)dlsym(m_pSnsLib, "gSnsimx464Obj");
    if (NULL == m_pSnsObj) {
        LOG_M_E(IMX464, "AX_VIN Get Sensor Object Failed!");
        return -1;
    }

    nRet = AX_VIN_RegisterSensor(nPipe, m_pSnsObj); /* pipe 0 */
    if (nRet) {
        LOG_M_E(IMX464, "AX_ISP Register Sensor Failed, ret=0x%x.", nRet);
        return nRet;
    }
    ...
    if (NULL != m_pSnsObj->pfn_sensor_set_bus_info) {
        ...
        if (0 != nRet) {
            LOG_M_E(IMX464, "Sensor set bus info Failed, ret=0x%x.", nRet);
            return nRet;
        }
        LOG_M(IMX464, "set sensor bus idx %d", tSnsBusInfo.I2cDev);
    } else {
        LOG_M(IMX464, "not support set sensor bus info!");
        return -1;
    }
...
AX_S32 CIMX464::UnRegisterSensor(AX_U8 nPipe)
{
...
AX_S32 CIMX464::RegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;
    ...
    if (nRet) {
        LOG_M_E(IMX464, "AX_ISP_RegisterAwbLibCallback Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CIMX464::UnRegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;

    nRet = AX_ISP_UnRegisterAwbLibCallback(nPipe);
    if (nRet) {
        LOG_M_E(IMX464, "AX_ISP Unregister Sensor Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CIMX464::SetFps(AX_U8 nPipe, AX_F32 fFrameRate)
{
    ...
    if (m_pSnsObj && m_pSnsObj->pfn_sensor_set_fps) {
        ...
        if (nRet) {
            LOG_M_E(IMX464, "[%d] Set Sensor Fps  to %.1f failed, ret=0x%x", nPipe, fFrameRate, nRet);
            return nRet;
        }
        else {
            LOG_M(IMX464, "[%d] Set Sensor Fps to %.1f", nPipe, fFrameRate);
        }
...
AX_BOOL CIMX464::HdrModeSupport(AX_U8 nPipe)
{
    return AX_TRUE;
}
