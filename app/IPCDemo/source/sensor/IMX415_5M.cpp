...
#define IMX415_5M "IMX415_5M"

extern COptionHelper gOptions;

CIMX415_5M::CIMX415_5M(SENSOR_CONFIG_T tSensorConfig)
...
CIMX415_5M::~CIMX415_5M(AX_VOID)
{
}

AX_VOID CIMX415_5M::InitSnsAttr()
{
    /* Referenced by AX_VIN_SetSnsAttr */
    m_tSnsAttr.nWidth               = 2904;
    m_tSnsAttr.nHeight              = 1620;
...
AX_VOID CIMX415_5M::InitMipiRxAttr()
{
...
AX_VOID CIMX415_5M::InitDevAttr()
{
...
AX_VOID CIMX415_5M::InitPipeAttr()
{
...
AX_VOID CIMX415_5M::InitChnAttr()
{
...
AX_S32 CIMX415_5M::RegisterSensor(AX_U8 nPipe)
{
    ...
    if (NULL == m_pSnsLib) {
        LOG_M_E(IMX415_5M, "Load sensor lib failed!");
        return -1;
    }

    m_pSnsObj = (AX_SENSOR_REGISTER_FUNC_T *)dlsym(m_pSnsLib, "gSnsimx415Obj");
    if (NULL == m_pSnsObj) {
        LOG_M_E(IMX415_5M, "AX_VIN Get Sensor Object Failed!");
        return -1;
    }

    nRet = AX_VIN_RegisterSensor(nPipe, m_pSnsObj); /* pipe 0 */
    if (nRet) {
        LOG_M_E(IMX415_5M, "AX_ISP Register Sensor Failed, ret=0x%x.", nRet);
        return nRet;
    }
    ...
    if (NULL != m_pSnsObj->pfn_sensor_set_bus_info) {
        ...
        if (0 != nRet) {
            LOG_M_E(IMX415_5M, "Sensor set bus info Failed, ret=0x%x.", nRet);
            return nRet;
        }
        LOG_M(IMX415_5M, "set sensor bus idx %d", tSnsBusInfo.I2cDev);
    } else {
        LOG_M(IMX415_5M, "not support set sensor bus info!");
        return -1;
    }
...
AX_S32 CIMX415_5M::UnRegisterSensor(AX_U8 nPipe)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
...
AX_S32 CIMX415_5M::RegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;
    ...
    if (nRet) {
        LOG_M_E(IMX415_5M, "AX_ISP_RegisterAwbLibCallback Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CIMX415_5M::UnRegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;

    nRet = AX_ISP_UnRegisterAwbLibCallback(nPipe);
    if (nRet) {
        LOG_M_E(IMX415_5M, "AX_ISP Unregister Sensor Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CIMX415_5M::SetFps(AX_U8 nPipe, AX_F32 fFrameRate)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
    ...
    if (m_pSnsObj && m_pSnsObj->pfn_sensor_set_fps) {
        ...
        if (nRet) {
            LOG_M_E(IMX415_5M, "[%d] Set Sensor Fps  to %.1f failed, ret=0x%x", nPipe, fFrameRate, nRet);
            return nRet;
        }
        else {
            LOG_M(IMX415_5M, "[%d] Set Sensor Fps to %.1f", nPipe, fFrameRate);
        }
...
AX_BOOL CIMX415_5M::HdrModeSupport(AX_U8 nPipe)
{
    return AX_TRUE;
}
