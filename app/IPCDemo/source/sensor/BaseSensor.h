...
typedef struct _SENSOR_HOTNOISEBALANCE_T
{
    ...
    _SENSOR_HOTNOISEBALANCE_T() {
        memset(this, 0x00, sizeof(_SENSOR_HOTNOISEBALANCE_T));
    }
...
typedef struct _SENSOR_CONFIG_T
{
    ...
    AX_CHAR             aSdrHotNoiseNormalModeBin[SENSOR_BIN_PATH_LEN];
    AX_CHAR             aSdrHotNoiseBalanceModeBin[SENSOR_BIN_PATH_LEN];
    AX_CHAR             aHdrHotNoiseNormalModeBin[SENSOR_BIN_PATH_LEN];
    AX_CHAR             aHdrHotNoiseBalanceModeBin[SENSOR_BIN_PATH_LEN];
    AX_CHAR             aIqSdrBin[SENSOR_BIN_PATH_LEN];
    AX_CHAR             aIqHdrBin[SENSOR_BIN_PATH_LEN];
    AX_BOOL             bTuning;
    AX_U32              nTuningPort;
    AX_BOOL             bEnableEIS;
...
public:
    ...
    AX_VOID ResetSensor(AX_U8 nPipe);
    ...
    AX_BOOL Load3ABinParams(const std::string &strBinName);
...
private:
    ...
    AX_U32  GetAdditionYUVBLKCount(AX_U32 nChn);
    AX_BOOL LoadIqBin();
...
private:
    AX_VOID *m_pLensLenDCIrisLib[DEF_VIN_PIPE_MAX_NUM]{nullptr};
    AX_VOID *m_pLensLenIrisLib[DEF_VIN_PIPE_MAX_NUM]{nullptr};
};
