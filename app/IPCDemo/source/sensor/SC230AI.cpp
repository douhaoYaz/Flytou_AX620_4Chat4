...
#define SC230AI "SC230AI"

extern COptionHelper gOptions;

CSC230AI::CSC230AI(SENSOR_CONFIG_T tSensorConfig)
    : CBaseSensor(tSensorConfig)
{
}

CSC230AI::~CSC230AI(AX_VOID)
{
}

AX_VOID CSC230AI::InitSnsAttr()
{
...
AX_VOID CSC230AI::InitMipiRxAttr()
{
...
AX_VOID CSC230AI::InitChnAttr()
{
    ...
    m_tChnAttr.tChnAttr[0].nDepth = YUV_SC230AI_DEPTH;
    ...
    m_tChnAttr.tChnAttr[1].nDepth = YUV_SC230AI_DEPTH;
    ...
    m_tChnAttr.tChnAttr[2].nDepth = YUV_SC230AI_DEPTH;
}

AX_S32 CSC230AI::RegisterSensor(AX_U8 nPipe)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);

    AX_S32 nRet = 0;

    m_pSnsLib = dlopen("libsns_sc230ai.so", RTLD_LAZY);
    if (NULL == m_pSnsLib) {
        LOG_M_E(SC230AI, "Load sensor lib failed!");
        return -1;
    }

    m_pSnsObj = (AX_SENSOR_REGISTER_FUNC_T *)dlsym(m_pSnsLib, "gSnssc230aiObj");
    if (NULL == m_pSnsObj) {
        LOG_M_E(SC230AI, "AX_VIN Get Sensor Object Failed!");
        return -1;
    }

    nRet = AX_VIN_RegisterSensor(nPipe, m_pSnsObj); /* pipe 0 */
    if (nRet) {
        LOG_M_E(SC230AI, "AX_ISP Register Sensor Failed, ret=0x%x.", nRet);
        return nRet;
    }
    ...
    if (NULL != m_pSnsObj->pfn_sensor_set_bus_info) {
        ...
        if (0 != nRet) {
            LOG_M_E(SC230AI, "Sensor set bus info Failed, ret=0x%x.", nRet);
            return nRet;
        }
        LOG_M(SC230AI, "set sensor bus idx %d", tSnsBusInfo.I2cDev);
    } else {
        LOG_M(SC230AI, "not support set sensor bus info!");
        return -1;
    }
...
AX_S32 CSC230AI::UnRegisterSensor(AX_U8 nPipe)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
...
AX_S32 CSC230AI::RegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;
    ...
    if (nRet) {
        LOG_M_E(SC230AI, "AX_ISP_RegisterAwbLibCallback Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CSC230AI::UnRegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;

    nRet = AX_ISP_UnRegisterAwbLibCallback(nPipe);
    if (nRet) {
        LOG_M_E(SC230AI, "AX_ISP Unregister Sensor Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CSC230AI::SetFps(AX_U8 nPipe, AX_F32 fFrameRate)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
    ...
    if (m_pSnsObj && m_pSnsObj->pfn_sensor_set_fps) {
        ...
        if (nRet) {
            LOG_M_E(SC230AI, "[%d] Set Sensor Fps  to %.1f failed, ret=0x%x", nPipe, fFrameRate, nRet);
            return nRet;
        }
        else {
            LOG_M(SC230AI, "[%d] Set Sensor Fps to %.1f", nPipe, fFrameRate);
        }
...
AX_BOOL CSC230AI::HdrModeSupport(AX_U8 nPipe)
{
    return AX_TRUE;
}
