...
#define IMX334 "IMX334"

extern COptionHelper gOptions;

CIMX334::CIMX334(SENSOR_CONFIG_T tSensorConfig)
...
CIMX334::~CIMX334(AX_VOID)
{
}

AX_VOID CIMX334::InitSnsAttr()
{
...
AX_VOID CIMX334::InitMipiRxAttr()
{
...
AX_VOID CIMX334::InitDevAttr()
{
    ...
    m_tDevAttr.tDevImgRgn.nStartY = 10;
...
AX_S32 CIMX334::RegisterSensor(AX_U8 nPipe)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);

    AX_S32 nRet = 0;

    m_pSnsLib = dlopen("libsns_imx334.so", RTLD_LAZY);
    if (NULL == m_pSnsLib) {
        LOG_M_E(IMX334, "Load sensor lib failed!");
        return -1;
    }

    m_pSnsObj = (AX_SENSOR_REGISTER_FUNC_T *)dlsym(m_pSnsLib, "gSnsimx334Obj");
    if (NULL == m_pSnsObj) {
        LOG_M_E(IMX334, "AX_VIN Get Sensor Object Failed!");
        return -1;
    }

    nRet = AX_VIN_RegisterSensor(nPipe, m_pSnsObj); /* pipe 0 */
    if (nRet) {
        LOG_M_E(IMX334, "AX_ISP Register Sensor Failed, ret=0x%x.", nRet);
        return nRet;
    }
    ...
    if (NULL != m_pSnsObj->pfn_sensor_set_bus_info) {
        ...
        if (0 != nRet) {
            LOG_M_E(IMX334, "Sensor set bus info Failed, ret=0x%x.", nRet);
            return nRet;
        }
        LOG_M(IMX334, "set sensor bus idx %d", tSnsBusInfo.I2cDev);
    } else {
        LOG_M(IMX334, "not support set sensor bus info!");
        return -1;
    }
...
AX_S32 CIMX334::UnRegisterSensor(AX_U8 nPipe)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
...
AX_S32 CIMX334::RegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;
    ...
    if (nRet) {
        LOG_M_E(IMX334, "AX_ISP_RegisterAwbLibCallback Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CIMX334::UnRegisterAwbAlgLib(AX_U8 nPipe)
{
    AX_S32 nRet = 0;

    nRet = AX_ISP_UnRegisterAwbLibCallback(nPipe);
    if (nRet) {
        LOG_M_E(IMX334, "AX_ISP Unregister Sensor Failed, ret=0x%x", nRet);
        return nRet;
    }
...
AX_S32 CIMX334::SetFps(AX_U8 nPipe, AX_F32 fFrameRate)
{
    std::unique_lock<std::mutex> lck(m_mtxSnsObj);
    ...
    if (m_pSnsObj && m_pSnsObj->pfn_sensor_set_fps) {
        ...
        if (nRet) {
            LOG_M_E(IMX334, "[%d] Set Sensor Fps  to %.1f failed, ret=0x%x", nPipe, fFrameRate, nRet);
            return nRet;
        }
        else {
            LOG_M(IMX334, "[%d] Set Sensor Fps to %.1f", nPipe, fFrameRate);
        }
...
AX_BOOL CIMX334::HdrModeSupport(AX_U8 nPipe)
{
    return AX_TRUE;
}
