...
public:
    ...
    AX_BOOL StartHotNoiseBalance(AX_VOID);
    AX_VOID ResetHotNoiseBalance(AX_VOID);
    AX_BOOL GetSnsTemperature(AX_F32 &fTemperature);
    AX_U8 GetShutterMode(AX_VOID);
    AX_BOOL SetShutterMode(AX_U8 nShutterMode);
...
private:
    ...
    AX_BOOL ProcessThermal(AX_F32 fThermal, SENSOR_CONFIG_T &tSensorCfg);
...
private:
    ...
    // hot noise balance
    mutex m_mtxHotNoiseBalance;
    AX_BOOL m_bHotNoiseBalanceThreadRunning;
    SENSOR_HOTNOISEBALANCE_T m_tHotNoiseBalance;
};
