...
AX_BOOL CJsonCfgParser::GetVideoCfgByID(const AX_U32 &nID, VIDEO_CONFIG_T &stOutCfg)
{
    ...
    std::string strRCType = objMediaIns["type"].get<std::string>();
    ...
    picojson::object rc = objMediaCodec["rc"].get<picojson::object>();
    picojson::object cbr = rc["CBR"].get<picojson::object>();
    stOutCfg.stRCInfoCBR.nMinQp = cbr["min_qp"].get<double>();
    stOutCfg.stRCInfoCBR.nMaxQp = cbr["max_qp"].get<double>();
    stOutCfg.stRCInfoCBR.nMinIQp = cbr["min_iqp"].get<double>();
    stOutCfg.stRCInfoCBR.nMaxIQp = cbr["max_iqp"].get<double>();
    stOutCfg.stRCInfoCBR.nMinIProp = cbr["min_i_prop"].get<double>();
    stOutCfg.stRCInfoCBR.nMaxIProp = cbr["max_i_prop"].get<double>();
    stOutCfg.stRCInfoCBR.nIntraQpDelta = cbr["intra_qp_delta"].get<double>();
    picojson::object vbr = rc["VBR"].get<picojson::object>();
    stOutCfg.stRCInfoVBR.nMinQp = vbr["min_qp"].get<double>();
    stOutCfg.stRCInfoVBR.nMaxQp = vbr["max_qp"].get<double>();
    stOutCfg.stRCInfoVBR.nMinIQp = vbr["min_iqp"].get<double>();
    stOutCfg.stRCInfoVBR.nMaxIQp = vbr["max_iqp"].get<double>();
    stOutCfg.stRCInfoVBR.nMinIProp = vbr["min_i_prop"].get<double>();
    stOutCfg.stRCInfoVBR.nMaxIProp = vbr["max_i_prop"].get<double>();
    stOutCfg.stRCInfoVBR.nIntraQpDelta = vbr["intra_qp_delta"].get<double>();
...
AX_BOOL CJsonCfgParser::GetCameraCfg(SENSOR_CONFIG_T &stOutCfg, SENSOR_ID_E eSensorID)
{
    ...
    // hot noise normal mode bin
    if (objSensor.end() != objSensor.find("sdr_hot_noise_normal_mode_bin")) {
        strncpy(stOutCfg.aSdrHotNoiseNormalModeBin, objSensor["sdr_hot_noise_normal_mode_bin"].get<std::string>().c_str(), SENSOR_BIN_PATH_LEN - 1);
    }

    // hot noise hotbalance mode bin
    if (objSensor.end() != objSensor.find("sdr_hot_noise_balance_mode_bin")) {
        strncpy(stOutCfg.aSdrHotNoiseBalanceModeBin, objSensor["sdr_hot_noise_balance_mode_bin"].get<std::string>().c_str(), SENSOR_BIN_PATH_LEN - 1);
    }

    // hot noise normal mode bin
    if (objSensor.end() != objSensor.find("hdr_hot_noise_normal_mode_bin")) {
        strncpy(stOutCfg.aHdrHotNoiseNormalModeBin, objSensor["hdr_hot_noise_normal_mode_bin"].get<std::string>().c_str(), SENSOR_BIN_PATH_LEN - 1);
    }

    // hot noise hotbalance mode bin
    if (objSensor.end() != objSensor.find("hdr_hot_noise_balance_mode_bin")) {
        strncpy(stOutCfg.aHdrHotNoiseBalanceModeBin, objSensor["hdr_hot_noise_balance_mode_bin"].get<std::string>().c_str(), SENSOR_BIN_PATH_LEN - 1);
    }

    if (objSensor.end() != objSensor.find("sdr_iq_bin")) {
        strncpy(stOutCfg.aIqSdrBin, objSensor["sdr_iq_bin"].get<std::string>().c_str(), SENSOR_BIN_PATH_LEN - 1);
    }

    if (objSensor.end() != objSensor.find("hdr_iq_bin")) {
        strncpy(stOutCfg.aIqHdrBin, objSensor["hdr_iq_bin"].get<std::string>().c_str(), SENSOR_BIN_PATH_LEN - 1);
    }
...
AX_BOOL CJsonCfgParser::GetAiCfg(const AX_U32 &nID, AI_CONFIG_T &stOutCfg)
{
    ...
    if (objMediaIns.end() != objMediaIns.find("frame_depth")) {
        stOutCfg.nFrameDepth = objMediaIns["frame_depth"].get<double>();
    }

    if (objMediaIns.end() != objMediaIns.find("cache_depth")) {
        stOutCfg.nCacheDepth = objMediaIns["cache_depth"].get<double>();
    }
...
AX_BOOL CJsonCfgParser::GetCameraHotNoiseBalanceCfg(SENSOR_HOTNOISEBALANCE_T &stOutCfg, SENSOR_ID_E eSensorID)
{
    picojson::array &arrModules = m_cfgRoot["modules"].get<picojson::array>();
    picojson::object objCameraHotNoiseBalance;

    for (size_t i = 0; i < arrModules.size(); i++) {
        auto &it = arrModules[i].get<picojson::object>();
        if (it["type"].get<std::string>() == "camerahotnoisebalance") {
            objCameraHotNoiseBalance = it;
            break;
        }
    }

    if (objCameraHotNoiseBalance.empty()) {
        LOG_M(JSON, "module(camerahotnoisebalance) not configured, will use default");
        return AX_FALSE;
    }

    picojson::array &arrCameraHotNoiseBalanceIns = objCameraHotNoiseBalance["instance"].get<picojson::array>();
    picojson::object objSensor;
    bool bCameraHotNoiseBalanceFound = false;
    for (size_t i = 0; i < arrCameraHotNoiseBalanceIns.size(); i++) {
        auto &it = arrCameraHotNoiseBalanceIns[i].get<picojson::object>();
        if (((SENSOR_ID_E)it["id"].get<double>() == eSensorID)) {
            objSensor = it;
            bCameraHotNoiseBalanceFound = true;
            break;
        }
    }

    if (!bCameraHotNoiseBalanceFound) {
        LOG_M(JSON, "Camera hot noise balance configuration not found!");
        return AX_FALSE;
    }

    stOutCfg.bEnable = (AX_BOOL)objSensor["enable"].get<bool>();
    stOutCfg.fThersholdM = (AX_F32)objSensor["median"].get<double>();
    stOutCfg.fThersholdL = (AX_F32)objSensor["low"].get<double>();
    stOutCfg.fGap = (AX_F32)objSensor["gap"].get<double>();
