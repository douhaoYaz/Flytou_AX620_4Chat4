...
typedef struct _VIDEO_RC_SET_INFO_T
{
    RcType eRCType;
    AX_U32 nMinIProp;
    AX_U32 nMaxIProp;

    _VIDEO_RC_SET_INFO_T() {
        eRCType = RcType::VENC_RC_CBR;
        nMinIProp = 10;
        nMaxIProp = 40;
    }
} VIDEO_RC_SET_INFO_T;
...
public:
    ...
    VIDEO_ATTR_T    GetVideo(AX_U32 nChn);
    AX_VOID         SetVideo(AX_U32 nChn, const VIDEO_ATTR_T& tVideo);
    VIDEO_RC_SET_INFO_T GetVideoRcSetInfo(AX_U32 nChn);
    AX_VOID         SetVideoRcSetInfo(AX_U32 nChn, const VIDEO_RC_SET_INFO_T& tRcSetInfo);
    AX_BOOL         GetVideoRcSetInfoStr(AX_U32 nChn, AX_CHAR* pOutBuf, AX_U32 nSize);
...
private:
    ...
    std::map<AX_U32, VIDEO_RC_SET_INFO_T> m_mapRcSetInfo;
};
