...
typedef struct _DETECT_RESULT_T {
    ...
    AX_U32 nCycleSize;
    AI_Detection_PoseResult_t tPoses[MAX_DECT_BOX_COUNT];
    AX_U32 nPoseSize;
...
typedef struct _AX_LENS_DRIVE_T {
    std::string strLensDCIrisLibName;
    std::string strLensDCIrisObjName;
    std::string strLensIrisLibName;
    std::string strLensIrisObjName;
} AX_LENS_DRIVE_T;
...
public:
    ...
    AX_BOOL IsActivedSceneChangeDetect(void) const;
    ...
    AX_VOID ActiveSceneChangeDetect(AX_BOOL bActive);
    ...
    AX_BOOL GetHotBalanceTest() const;
    AX_BOOL GetSnsHotNoiseBalanceTest() const;
    ...
    /* lens drive config */
    AX_LENS_DRIVE_T GetLensCfg() const;

    AX_BOOL IsLoadNoneBiasBinSupport(void) const;
    AX_VOID SetLoadNoneBiasBinSupport(AX_BOOL bSupport);
    AX_BOOL IsLoadNoneBiasBinEnable(void) const;
    AX_VOID SetLoadNoneBiasBinEnable(AX_BOOL bEnable);
...
private:
    ...
    AX_BOOL m_bMirror;
    AX_BOOL m_bFlip;
    AX_BOOL m_bHotBalanceTest;
    AX_BOOL m_bSnsHotNoiseBalanceTest;
    AX_BOOL m_bNoneBiasSupport;
    AX_BOOL m_bEnableNoneBias;
    std::string m_strJsonCfgFile;
    std::string m_strMp4SavedPath;
    std::string m_strDetectConfigPath;
    std::string m_strTtfFile;
    std::string m_strSnsName;
    DETECT_RESULT_T m_arrDetectResult[MAX_SNS_NUM];

    std::mutex m_mtxOption;
    AX_BOOL m_bActiveSceneChangeDetect{AX_FALSE};

    inifile::IniFile m_ini;
    AX_LENS_DRIVE_T m_stLensDriveCfg;
};
