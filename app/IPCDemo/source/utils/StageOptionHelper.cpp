    ...
    for (const auto& item : g_tEPOptions) {
        ...
        if (item.eEPType == E_END_POINT_VENC) {
            ...
            if (CConfigParser().GetInstance()->GetVideoCfgByID(item.nInnerIndex, tVEncConfig)) {
                ...
                VIDEO_RC_SET_INFO_T rcSetInfo;
                RC_INFO_T stRCInfo;
                rcSetInfo.eRCType = tVEncConfig.eRCType;
                if (rcSetInfo.eRCType == RcType::VENC_RC_CBR) {
                    stRCInfo = tVEncConfig.stRCInfoCBR;
                } else {
                    stRCInfo = tVEncConfig.stRCInfoVBR;
                }
                rcSetInfo.nMinIProp = stRCInfo.nMinIProp;
                rcSetInfo.nMaxIProp = stRCInfo.nMaxIProp;
                m_mapRcSetInfo.emplace(item.nChannel, rcSetInfo);
            }
        }
...
AX_BOOL CStageOptionHelper::InitAiAttr()
{
    ...
    m_tAiAttr.tEvents.tSCD.nEnable = gOptions.IsActivedSceneChangeDetect();
    AX_S32 scdThrd;
    AX_S32 scdConfidence;
    CSCD::GetInstance()->GetThreshold(scdThrd, scdConfidence);
    m_tAiAttr.tEvents.tSCD.nThrsHoldY = (AX_U16)scdThrd;
    m_tAiAttr.tEvents.tSCD.nConfidence = (AX_U16)scdConfidence;
...
AX_VOID CStageOptionHelper::SetCamera(const CAMERA_ATTR_T& tCamera)
{
    ...
    m_tCamera.nNoneBias             = tCamera.nNoneBias;
}

AX_VOID CStageOptionHelper::SetVideo(AX_U32 nChn, const VIDEO_ATTR_T& tVideo)
{
    std::lock_guard<std::mutex> lck(m_mtxOption);
    m_mapVideo[nChn] = tVideo;
}

VIDEO_ATTR_T CStageOptionHelper::GetVideo(AX_U32 nChn)
{
    std::lock_guard<std::mutex> lck(m_mtxOption);
    return m_mapVideo[nChn];
}

VIDEO_RC_SET_INFO_T CStageOptionHelper::GetVideoRcSetInfo(AX_U32 nChn)
{
    std::lock_guard<std::mutex> lck(m_mtxOption);
    return m_mapRcSetInfo[nChn];
}

AX_VOID CStageOptionHelper::SetVideoRcSetInfo(AX_U32 nChn, const VIDEO_RC_SET_INFO_T& tRcSetInfo)
{
    std::lock_guard<std::mutex> lck(m_mtxOption);
    m_mapRcSetInfo[nChn] = tRcSetInfo;
}
...
AX_BOOL CStageOptionHelper::GetCameraStr(AX_CHAR* pOutBuf, AX_U32 nSize)
{
    ...
    AX_S32 nCount = snprintf(pOutBuf, nSize,
            "{sns_work_mode: %d, rotation: %d, mirror: %s, flip: %s, framerate: %d, daynight: %d, nr_mode: %s, eis_support: %s, eis: %s, nonebias: %s, nonebias_support: %s}",
            ...
            m_tCamera.nNoneBias ? "true" : "false",
            gOptions.IsLoadNoneBiasBinSupport() ? "true" : "false");
...
{
    std::lock_guard<std::mutex> lck(m_mtxOption);
    const AX_U8 nSensorSize = E_SNS_TYPE_MAX;
...
AX_BOOL CStageOptionHelper::GetVideoRcSetInfoStr(AX_U32 nChn, AX_CHAR* pOutBuf, AX_U32 nSize)
{
    if (nullptr == pOutBuf || 0 == nSize) {
        return AX_FALSE;
    }
    ...
    VIDEO_RC_SET_INFO_T *pRcSetInfo = &m_mapRcSetInfo[nChn];

    AX_S32 nCount = snprintf(pOutBuf, nSize,
            "{rc_type: %d, min_iprop: %d, max_iprop: %d}",
            (AX_U32)pRcSetInfo->eRCType,
            pRcSetInfo->nMinIProp,
            pRcSetInfo->nMaxIProp);
...
AX_BOOL CStageOptionHelper::GetEventsStr(AX_CHAR* pOutBuf, AX_U32 nSize)
{
    ...
    AX_S32 nCount = snprintf(pOutBuf, nSize, "motion_detect: {enable: %s, threshold_y: %d, confidence: %d}, \
        ...
        ADAPTER_INT2BOOLSTR(m_tAiAttr.tEvents.tSCD.nEnable),
        m_tAiAttr.tEvents.tSCD.nThrsHoldY,
        m_tAiAttr.tEvents.tSCD.nConfidence);
