...
if [ "$PROJECT" = "AX620_nand" ] || [ "$PROJECT" = "AX620_38board" ] || [ "$PROJECT" = "AX620U_38board" ] || [ "$PROJECT" = "AX620U_nand" ] ; then
	...
	python3 $TOOL_PATH -p $CHIP_GROUP -v $VERSION_EXT -x $XML_PATH -o $AXP_PATH $EIP_PATH $FDL1_PATH $FDL2_PATH $UBOOT_PATH $DTB_PATH $BOOT_PATH $ROOTFS_PATH $PARAM_PATH $SOC_PATH $OPT_PATH $SPL_PATH
	echo "make axp done"
fi
else
